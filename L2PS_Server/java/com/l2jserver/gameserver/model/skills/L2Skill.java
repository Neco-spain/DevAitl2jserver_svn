...
public abstract class L2Skill implements IChanceSkillTrigger
{
	...
	public static final boolean checkForAreaOffensiveSkills(L2Character caster, L2Character target, L2Skill skill, boolean sourceInArena)
	{
		...
		if (player != null)
		{
			if (targetPlayer != null)
			{
				...
				if (Interface.isParticipating(player.getObjectId()) && Interface.isParticipating(targetPlayer.getObjectId()))
				{
					if (Interface.areTeammates(player.getObjectId(), targetPlayer.getObjectId()))
					{
						return false;
					}
				}
	...
	public static final boolean checkForAreaFriendlySkills(L2PcInstance activeChar, L2Character target, L2Skill skill)
	{
		...
		if ((target instanceof L2Playable))
		{
			...
			if ((activeChar.isInOlympiadMode()) && (pTarget.isInOlympiadMode()) && (activeChar.getOlympiadSide() != pTarget.getOlympiadSide()) && (activeChar.isOlympiadStart()))
			{
				return false;
			}
			if ((activeChar.getSiegeState() > 0) && (activeChar.isInsideZone(ZoneId.SIEGE)) && (activeChar.getSiegeState() != pTarget.getSiegeState()) && (pTarget.getSiegeSide() != activeChar.getSiegeSide()))
			{
				return false;
			}
			if ((activeChar.isInSameParty(pTarget)) || (activeChar.isInSameChannel(pTarget)) || (activeChar.isInSameClan(pTarget)) || (activeChar.isInSameAlly(pTarget)))
			{
				return true;
			}
			if ((pTarget.isAutoAttackable(activeChar)) || (pTarget.isInsideZone(ZoneId.PVP)) || (activeChar.isInSameClanWar(pTarget)) || (pTarget.getKarma() != 0) || (pTarget.getPvpFlag() != 0))
			{
				return false;
			}
