...
import com.l2jserver.gameserver.model.skills.funcs.FuncGet;
...
public class HeartInfinityAttack extends Quest
{
	...
	protected int enterInstance(L2PcInstance player, String template, int[] coords)
	{
		...
		L2Npc npc = addSpawn(32536, -179376, 206111, -15538, 16384, false, 0, false, ((HIAWorld) world).instanceId);
	...
	protected void conquestBegins(HIAWorld world)
	{
		...
		world.ekimus = addSpawn(EKIMUS, -179537, 208854, -15504, 16384, false, 0, false, world.instanceId);
		...
		world.hounds.add(addSpawn(HOUND, -179224, 209624, -15504, 16384, false, 0, false, world.instanceId));
		world.hounds.add(addSpawn(HOUND, -179880, 209464, -15504, 16384, false, 0, false, world.instanceId));
	...
	public String onAggroRangeEnter(L2Npc npc, L2PcInstance player, boolean isSummon)
	{
		...
		if (tmpworld instanceof HIAWorld)
		{
			...
			if (npc.getNpcId() == 18668)
			{
				...
				{
					spawnNpc(mobs[Rnd.get(mobs.length)], npc.getLocation(), 0, world.instanceId);
				}
				npc.doDie(npc);
			}
		}
	...
	private class TumorRevival implements Runnable
	{
		...
		public void run()
		{
			...
			L2Npc alivetumor = spawnNpc(ALIVETUMOR, _deadTumor.getLocation(), 0, _world.instanceId);
	...
	private class RegenerationCoffinSpawn implements Runnable
	{
		...
		public void run()
		{
			...
			for (int i = 0; i < 4; i++)
			{
				L2Npc worm = spawnNpc(COFFIN, _deadTumor.getLocation(), 0, _world.instanceId);
