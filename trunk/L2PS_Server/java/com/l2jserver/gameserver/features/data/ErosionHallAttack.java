...
public class ErosionHallAttack extends Quest
{
		...
		if (tmpworld instanceof HEWorld)
		{
			...
			if (npc.getNpcId() == TUMOR)
			{
				...
				if ((world.tumorCount == 0) && (world.cohemenes == null))
				{
					...
					L2Npc n = addSpawn(spawn[0], spawn[1], spawn[2], spawn[3], spawn[4], false, 0, false, world.instanceId);
	...
	private class TumorRevival implements Runnable
	{
		...
		public void run()
		{
			...
			L2Npc tumor = spawnNpc(TUMOR, _deadTumor.getLocation(), 0, _world.instanceId);
	...
	private class RegenerationCoffinSpawn implements Runnable
	{
		...
		public void run()
		{
			...
			for (int i = 0; i < 4; i++)
			{
				L2Npc worm = spawnNpc(18710, _deadTumor.getLocation(), 0, _world.instanceId);
