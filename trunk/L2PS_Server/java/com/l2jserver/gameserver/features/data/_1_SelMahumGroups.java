...
public class _1_SelMahumGroups extends AbstractNpcAI
{
	...
	{
		"Who is mucking with my recruits!?!",
	...
	{
		"They done killed da Sarge... Run!!",
		"Don't Panic... Okay, Panic!"
	};
	
	protected static List<L2Spawn> _spawns = new FastList<>(); // all Mahum's spawns are stored here
	protected static TIntHashSet _scheduledReturnTasks = new TIntHashSet(); // Used to track scheduled Return Tasks
	
	public _1_SelMahumGroups(int questId, String name, String descr)
	{
		super(name, descr);
				...
				if (!npc.isBusy() && (npc.getAI().getIntention() == CtrlIntention.AI_INTENTION_ACTIVE) && (npc.getX() == npc.getSpawn().getLocx()) && (npc.getY() == npc.getSpawn().getLocy()))
				{
					...
					if (idx <= (CHIEF_SOCIAL_ACTIONS.length - 1))
					{
						...
						if (zone != null)
						{
							...
							{
								if ((ch != null) && !ch.isDead() && (ch instanceof L2MonsterInstance) && !((L2MonsterInstance) ch).isBusy() && Util.contains(MAHUM_SOLDIERS, ((L2MonsterInstance) ch).getNpcId()) && (ch.getAI().getIntention() == CtrlIntention.AI_INTENTION_ACTIVE) && (ch.getX() == ((L2MonsterInstance) ch).getSpawn().getLocx()) && (ch.getY() == ((L2MonsterInstance) ch).getSpawn().getLocy()))
								{
									ch.broadcastSocialAction(SOLDIER_SOCIAL_ACTIONS[idx]);
								}
							}
						}
					}
	...
	protected L2ZoneType getZone(L2Npc npc)
	{
	...
	public static void main(String[] args)
	{
		...
		new _1_SelMahumGroups(-1, "sel_mahums", "data");
	}
}