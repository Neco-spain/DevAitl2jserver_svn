...
public class _4_SeerUgoros extends AbstractNpcAI
{
	// Boss attack task
	...
	private void broadcastInRegion(L2Npc npc, String _text)
	{
		...
		int region = MapRegionManager.getInstance().getMapReg(npc.getX(), npc.getY());
		for (L2PcInstance player : L2World.getInstance().getAllPls())
		{
			if (region == MapRegionManager.getInstance().getMapReg(player.getX(), player.getY()))
			{
	...
	public _4_SeerUgoros(String name, String descr)
	{
		super(name, descr);
	...
	{
		new _4_SeerUgoros("SeerUgoros", "ai");
	}
}
