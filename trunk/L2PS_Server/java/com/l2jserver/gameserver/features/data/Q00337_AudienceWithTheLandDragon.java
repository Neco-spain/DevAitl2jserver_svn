...
public class Q00337_AudienceWithTheLandDragon extends Quest
{
			...
			if (npcInfo[0] == npcId)
			{
				...
				if (percentHp < 66.0D)
				{
					...
					if (spawn)
					{
						...
						{
							L2Npc mob = addSpawn(npcInfo[5], npc.getX() + Rnd.get(-150, 150), npc.getY() + Rnd.get(-150, 150), npc.getZ(), npc.getHeading(), true, 60000L, false);
							mob.setRunning();
		...
		switch (npcId)
		{
			...
			case BLOOD_QUEEN:
				if ((cond == 1) && (Rnd.get(100) < 25))
				{
					if ((st.getInt("drop1") == 1) && (!st.hasQuestItems(REMAINS_OF_SACRIFIED)))
					{
						for (int i = 0; i < 10; i++)
						{
							addSpawn(SACRIFICE_OF_THE_SACRIFICED, npc.getX() + Rnd.get(-100, 100), npc.getY() + Rnd.get(-100, 100), npc.getZ(), npc.getHeading(), true, 60000L, false);
						}
					}
				}
				break;
			...
			case HARIT_LIZARDMAN_MATRIARCH:
				if ((cond == 1) && (Rnd.get(100) < 15))
				{
					if ((st.getInt("drop2") == 1) && (!st.hasQuestItems(TOTEM_OF_LAND_DRAGON)))
					{
						for (int i = 0; i < 3; i++)
						{
							addSpawn(HARIT_LIZARDMAN_ZEALOT, npc.getX() + Rnd.get(-50, 50), npc.getY() + Rnd.get(-50, 50), npc.getZ(), npc.getHeading(), true, 60000L, false);
						}
					}
				}
				break;
			...
			case CAVE_KEEPER:
				if ((cond == 4) && (Rnd.get(100) < 15))
				{
					if (!st.hasQuestItems(THIRD_FRAGMENT_OF_ABYSS_JEWEL))
					{
						addSpawn(ABYSSAL_JEWEL_3, npc.getX() + Rnd.get(-50, 50), npc.getY() + Rnd.get(-50, 50), npc.getZ(), npc.getHeading(), true, 60000L, false);
					}
				}
				break;
		}
	...
	{
		new Q00337_AudienceWithTheLandDragon(337, Q00337_AudienceWithTheLandDragon.class.getSimpleName(), "Audience With The Land Dragon");
	}
}