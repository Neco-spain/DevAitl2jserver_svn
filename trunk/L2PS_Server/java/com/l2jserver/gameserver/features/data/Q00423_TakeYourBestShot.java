...
public class Q00423_TakeYourBestShot extends Quest
{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		{
			if (event.equalsIgnoreCase("32744-04.html"))
			{
				st.setState(State.STARTED);
				st.set("cond", "1");
				st.playSound("ItemSound.quest_accept");
			}
			else if (event.equalsIgnoreCase("32744-quit.html"))
			{
				st.exitQuest(true);
			}
		}
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (npc.getNpcId() == _johnny)
		{
			...
			{
				case State.CREATED:
					QuestState _prev = player.getQuestState("Q00249_PoisonedPlainsOfTheLizardmen");
					if ((_prev != null) && (_prev.getState() == State.COMPLETED) && (player.getLevel() >= 82))
					{
						if (st.hasQuestItems(_seer_ugoros_pass))
						{
							htmltext = "32744-07.html";
						}
						else
						{
							htmltext = "32744-01.html";
						}
					}
					else
					{
						htmltext = "32744-00.html";
					}
					break;
				case State.STARTED:
					if (st.getInt("cond") == 1)
		...
		else if (npc.getNpcId() == _batracos)
		{
			if (st.getState() == State.CREATED)
			{
				if (st.hasQuestItems(_seer_ugoros_pass))
				{
					htmltext = "32740-05.html";
				}
				else
				{
					htmltext = "32740-00.html";
				}
			}
