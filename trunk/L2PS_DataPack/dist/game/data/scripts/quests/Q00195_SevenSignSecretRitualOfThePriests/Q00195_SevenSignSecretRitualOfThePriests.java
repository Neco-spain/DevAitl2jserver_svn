 ...
 */
package quests.Q00195_SevenSignSecretRitualOfThePriests;
...
public class Q00195_SevenSignSecretRitualOfThePriests extends Quest
{
	// INSTANCE DATA
	...
	public Q00195_SevenSignSecretRitualOfThePriests(int questId, String name, String descr)
	{
		...
		switch (st.getState())
		{
			...
			case State.CREATED:
				if (npcId == CLAUDIA_ATHEBALDT)
				{
					...
					{
						final QuestState qs = st.getPlayer().getQuestState("Q00194_SevenSignContractOfMammon");
						if (qs != null)
						{
							if (qs.isCompleted())
							{
		...
		if (!npc.isCastingNow())
		{
			switch (npc.getNpcId())
			{
				case 18834:
					...
					break;
				case 18835:
					npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc.getNpcId(), TEXT[1]));
					break;
				case 27351:
					npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc.getNpcId(), TEXT[2]));
					break;
			}
	...
	private void spawnMovingGuards(int instanceId)
	{
		...
		{
			ThreadPoolManager.getInstance().scheduleAi(new movmentTask(addSpawn(element[0], element[1], element[2], element[3], 0, false, 0, false, instanceId), element[1], element[2], element[3], element[4], element[5], element[6], getMoveDelay(element)), Rnd.get(1000, 5000));
		}
	}
	...
	{
		new Q00195_SevenSignSecretRitualOfThePriests(195, Q00195_SevenSignSecretRitualOfThePriests.class.getSimpleName(), "Seven Sing Secret Ritual Of The Priests");
	}
}