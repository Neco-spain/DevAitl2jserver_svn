...
public class NoChampionMobs extends AbstractNpcAI
{
	...
	public NoChampionMobs(int questId, String name, String descr)
	{
		super(name, descr);

		for (L2Spawn npc : SpawnTable.getInstance().getSpawnTable())
		{
			if (Util.contains(NoChampionList, npc.getNpcid()))
			{
				if(npc.getLastSpawn() != null)
					npc.getLastSpawn().setChampion(false);
			}
		}
	...
	{
		new NoChampionMobs(-1, "NoChampionMobs", "npc");
	}
}