...
public class ToTheMonastery extends Quest
{
		...
		if (tmpworld instanceof ToTheMonasteryWorld)
		{
			...
			if (npcId == POWERFUL_DEVICE_1)
			{
				if (st.getQuestItemsCount(STAFF_OF_BLESSING) > 0)
				{
					...
					addSpawn(18953, 45400, -246072, -6754, 49152, false, 0, false, world.getInstanceId());
					return null;
				}
			...
			if (npcId == POWERFUL_DEVICE_2)
			{
				if (st.getQuestItemsCount(SCROLL_OF_ABSTINENCE) > 0)
				{
					...
					addSpawn(18954, 48968, -249640, -6754, 32768, false, 0, false, world.getInstanceId());
					return null;
				}
				htmltext = "no-item.htm";
			}
			if (npcId == POWERFUL_DEVICE_3)
			{
				if (st.getQuestItemsCount(SWORD_OF_HOLYSPIRIT) > 0)
				{
					...
					addSpawn(18955, 45400, -253208, -6754, 16384, false, 0, false, world.getInstanceId());
					return null;
				}
				htmltext = "no-item.htm";
			}
			if (npcId == POWERFUL_DEVICE_4)
			{
				if (st.getQuestItemsCount(SHIELD_OF_SACRIFICE) > 0)
				{
					...
					addSpawn(18952, 41784, -249640, -6754, 0, false, 0, false, world.getInstanceId());
					return null;
				}
				htmltext = "no-item.htm";
			}
	...
	protected void SpawnFirstGroup(ToTheMonasteryWorld world)
	{
		...
		{
			L2Npc spawnedMob = addSpawn(27403, spawn[0], spawn[1], spawn[2], spawn[3], false, 0, false, world.getInstanceId());
	...
	protected void SpawnThirdGroup(ToTheMonasteryWorld world)
	{
		...
		{
			L2Npc spawnedMob = addSpawn(27404, spawn[0], spawn[1], spawn[2], spawn[3], false, 0, false, world.getInstanceId());
	...
	{
		new ToTheMonastery(-1, ToTheMonastery.class.getSimpleName(), "instances");
	}
}