...
public class Q00219_TestimonyOfFate extends Quest
{
	...
	public Q00219_TestimonyOfFate(int questId, String name, String descr)
	{
		...
		for (int[] element : DROPLIST_COND)
		{
			if ((cond == element[0]) && (npcId == element[2]))
			{
				if ((element[3] == 0) || (st.getQuestItemsCount(element[3]) > 0))
				{
					if (element[4] == 0)
					{
						st.dropQuestItems(element[4], element[5], element[5], element[6], element[7], true);
					}
					else if (st.dropQuestItems(element[4], element[5], element[5], element[6], element[7], true))
					{
	...
	{
		new Q00219_TestimonyOfFate(219, Q00219_TestimonyOfFate.class.getSimpleName(), "Testimony Of Fate");
	}
}
